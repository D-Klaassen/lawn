...
export function stepDrive(me: Driver, input: { throttle: number; turn: number; brake: boolean;
  street: number; grass: number; tow: number; stunned: boolean }, dt: number) {
  ...
  const drag = (5.5 + 1.9 * grass - 2.25 * street + me.driftGrip * 0.25 + me.brakePressure * 5 - boost * 0.7) * (stunned ? 3 : 1);
  ...
  const grip = 14 + street * (18 + me.driftGrip * 16);
